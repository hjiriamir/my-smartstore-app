...
import { Bell, CheckCircle, Clock, TrendingUp, Package, Users, User, LogOut, Menu } from "lucide-react"
...
export default function Dashboard() {
  ...
  const [magasin, setMagasin] = useState("Chargement...")
...
const MobileNavigation = () => (
    ...
    <SheetContent side={isRTL ? "right" : "left"} className="w-80 p-0">
      ...
      <div className="flex-1 overflow-y-auto p-2">
        <div className="space-y-2">
          {[
            { value: "dashboard", label: t("front.navBar.tabledDeBoard") },
            { value: "library", label: t("front.navBar.bibliotheque") },
            { value: "visualization", label: t("front.navBar.visualisation") },
            { value: "tracking", label: t("front.navBar.suivi") },
            { value: "communication", label: t("front.navBar.communication") },
            { value: "search", label: t("front.navBar.recherche") },
            { value: "support", label: t("front.navBar.support") },
          ].map((tab) => (
            <Button
              key={tab.value}
  ...
  const fetchNotifications = async (userId: number) => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/notification/getNotificationsByUser/${userId}`, {
  ...
  const markAsRead = async (notificationId: number) => {
    try {
      ...
      setNotifications(notifications.map((notif) => (notif.id === notificationId ? { ...notif, lu: true } : notif)))
    } catch (error) {
  ...
  const fetchRecentPlanograms = async (magasinId: string, userId: number) => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/planogram/planogramsRecent/${magasinId}/${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      ...
      const formattedPlanograms: Planogram[] = response.data.map((item: any) => ({
        ...
        status:
          item.statut === "à faire"
            ? t("front.dashboard.aImplementer")
  ...
  const fetchMagasinData = async (userId: number): Promise<string | null> => {
    ...
    try {
      ...
      const response = await axios.get<MagasinResponse>(`${API_BASE_URL}/magasins/getMagasinByUser/${userId}`, {
        headers: {
      ...
      if (response.data?.nom_magasin && response.data?.magasin_id) {
        ...
        return response.data.magasin_id.toString()
      } else {
  ...
  const fetchDashboardStats = async (magasinId: string, userId: number) => {
    try {
      ...
      if (typeof implementationRes.data.taux_implementation === "string") {
        implementationRate = Number.parseFloat(implementationRes.data.taux_implementation)
        ...
        implementationRate =
          (implementationRes.data.planograms_implementes / implementationRes.data.total_planograms) * 100
      }
    ...
    <div className="min-h-screen bg-gray-50 mt-14" dir={textDirection}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`flex justify-between items-center h-16 ${isRTL ? "flex-row-reverse" : ""}`}>
            <div className={`flex items-center ${isRTL ? "flex-row-reverse" : ""}`}>
              <Package className={`h-6 w-6 sm:h-8 sm:w-8 text-blue-600 ${isRTL ? "ml-2 sm:ml-3" : "mr-2 sm:mr-3"}`} />
              <h1 className="text-lg sm:text-xl font-semibold text-gray-900">Smart Store</h1>
            </div>

            <div className={`flex items-center space-x-2 sm:space-x-6 ${isRTL ? "space-x-reverse" : ""}`}>
              {/* Mobile Navigation */}
              <MobileNavigation />
              ...
              <Button variant="ghost" size="sm" className="relative">
                ...
                {notifications.filter((n) => !n.lu).length > 0 && (
                  <span className="absolute -top-1 -right-1 h-4 w-4 sm:h-5 sm:w-5 rounded-full bg-red-500 flex items-center justify-center text-xs text-white">
              ...
              <div className="hidden sm:flex items-center space-x-2 bg-gray-100 px-3 py-1 rounded-full">
                ...
                <span className="text-xs sm:text-sm font-medium">{t("front.support.magasin")} : {magasin}</span>
              </div>

              {/* User Menu */}
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="rounded-full w-8 h-8 bg-gray-100 hover:bg-gray-200">
                ...
                <DropdownMenuContent className="w-56" align={isRTL ? "start" : "end"} forceMount>
                  <DropdownMenuLabel className="font-normal">
                    <div className="flex flex-col space-y-1">
                      <p className="text-sm font-medium leading-none">{userData?.nom}</p>
                  ...
                  <DropdownMenuSeparator />
                  <DropdownMenuItem className="text-red-600 focus:text-red-700 focus:bg-red-50" onClick={handleLogout}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className={`hidden md:grid w-full grid-cols-7 mb-6 ${isRTL ? "text-right" : "text-left"}`}>
          ...
          {/* Mobile Store Info */}
          <div className="sm:hidden mb-4 p-3 bg-gray-100 rounded-lg">
            <div className="flex items-center space-x-2">
              <div className="w-6 h-6 bg-blue-600 rounded-full flex items-center justify-center">
                <Users className="h-3 w-3 text-white" />
              </div>
              <span className="text-sm font-medium" dir={textDirection}>{t("front.support.magasin")}  : {magasin}</span>
            </div>
          </div>
              ...
              <Card>
                ...
                >
                  <CardTitle className={`text-sm font-medium ${isRTL ? "text-right" : "text-left"}`}>
                    {t("front.dashboard.tacheEnAttent")}
                  </CardTitle>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className={isRTL ? "text-right" : "text-left"}>
                    {t("front.dashboard.recentPlanogram")}
                  </CardTitle>
                  <CardDescription className={isRTL ? "text-right" : "text-left"}>
                    {t("front.dashboard.recentPlanogramDesr")}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="h-[300px] sm:h-[400px] overflow-y-auto">
                    <div className="space-y-4 min-h-[250px] sm:min-h-[300px]">
                      {recentPlanograms.map((planogram) => (
                        <div
                          key={planogram.id}
                          className={`flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 border rounded-lg ${isRTL ? "flex-row-reverse" : ""}`}
                          style={{ minHeight: "80px" }}
                        >
                          <div className="flex-1 mb-2 sm:mb-0">
                            <h4 className={`font-medium ${isRTL ? "text-right" : "text-left"}`}>{planogram.name}</h4>
                    ...
                    <div className="space-y-4 min-h-[250px] sm:min-h-[300px]">
                      {notifications.length > 0 ? (
                        notifications.map((notification) => (
                          <div
                            key={notification.id}
                            className={`flex flex-col sm:flex-row items-start space-y-2 sm:space-y-0 sm:space-x-3 p-3 border rounded-lg cursor-pointer transition-colors ${
                          ...
                          >
                            <div
                              className={`w-2 h-2 rounded-full mt-2 ${
                                !notification.lu ? "bg-blue-500 animate-pulse" : "bg-gray-300"
                              }`}
                            />
                            <div className={`flex-1 ${isRTL ? "text-right" : "text-left"}`}>
                              <p className="text-sm font-medium">
                                {notification.type === "nouveau planogramme" && t("front.dashboard.nouvPlanogram")}
                                {notification.type === "confirmation requise" &&
                                  t("front.dashboard.confirmationRequise")}
