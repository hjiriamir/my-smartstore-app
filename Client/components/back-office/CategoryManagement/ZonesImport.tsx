...
export function ZonesImport({
  ...
  const [newZone, setNewZone] = useState<
    Omit<ZoneData, "date_creation" | "date_modification"> & {
      date_creation?: string
    }
  >({
    zone_id: "",
  ...
  const handleSaveEdit = () => {
    ...
    const updatedZones = importedZones.map((z) =>
      z.zone_id === editingZone.zone_id ? { ...z, [editField.key]: editField.value } : z,
    )
    setImportedZones(updatedZones)
    handleImport(updatedZones)
    setEditingZone(null)
  ...
  return (
    <div className="container max-w-4xl mx-auto px-4 py-6 md:px-6 lg:px-8" dir={textDirection}>
      ...
      <Card className="w-full">
        <CardHeader>
          <CardTitle className="text-xl md:text-2xl">{t("zoneImport.title")}</CardTitle>
          <CardDescription className="text-sm md:text-base">{t("zoneImport.description")}</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          {step === 1 && (
            <div className="space-y-4 md:space-y-6">
              <div className="space-y-1 md:space-y-2">
                <h3 className="text-base md:text-lg font-medium">{t("zoneImport.title")}</h3>
                <p className="text-xs md:text-sm text-muted-foreground">{t("zoneImport.description1")}</p>
              </div>
              <div
                className={`
              ...
              >
                {file ? (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs md:text-sm text-muted-foreground">{parsedData.length} zones détectées</p>
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={(e) => {
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-base md:text-lg font-medium">{t("zoneImport.select")}</p>
                    <p className="text-xs md:text-sm text-muted-foreground">{t("zoneImport.dragdrop")}</p>
                  </div>
                )}
                <Input
                  ref={fileInputRef}
          ...
          {step === 2 && (
            <div className="space-y-4 md:space-y-6">
              <div className="space-y-1 md:space-y-2">
                <h3 className="text-base md:text-lg font-medium">{t("zoneImport.mapColonne")}</h3>
                <p className="text-xs md:text-sm text-muted-foreground">{t("zoneImport.associer")}</p>
              </div>
              ...
              {validationErrors.length > 0 && (
                <Alert variant="destructive">
                  ...
                  <AlertTitle className="text-sm md:text-base">{t("productImport.validation.errors")}</AlertTitle>
          ...
          {step === 3 && (
            <div className="space-y-4 md:space-y-6">
              ...
              <div className="space-y-2 md:space-y-4">
                <h4 className="font-medium text-sm md:text-base">{t("zoneImport.resume")}</h4>
                <div className="border rounded-md p-3 md:p-4">
                  <p className="text-xs md:text-sm text-muted-foreground">{t("zoneImport.Aimporter")}</p>
              ...
              <div className="flex justify-between">
                ...
                <Button onClick={importZones} className="text-sm md:text-base">
                  {t("zoneImport.validImport")}
                </Button>
              </div>
            </div>
          )}
          {step === 3 && importProgress > 0 && (
            <div className="fixed inset-0 bg-background/80 flex items-center justify-center z-50 p-4">
              <Card className="w-full max-w-sm md:max-w-[400px]">
                <CardHeader>
                  <CardTitle className="text-lg md:text-xl">{t("productImport.importationEnCours")}</CardTitle>
                  <CardDescription className="text-sm md:text-base">{t("zoneImport.patient")}</CardDescription>
                </CardHeader>
                <CardContent className="space-y-3 md:space-y-4">
                  <Progress value={importProgress} className="h-2" />
                  <p className="text-center text-xs md:text-sm">
                    {importProgress < 100 ? t("zoneImport.processing") : t("zoneImport.complete")}
                  </p>
                </CardContent>
              </Card>
            </div>
          ...
          {showCompletionPopup && (
            <div className="fixed inset-0 bg-background/80 flex items-center justify-center z-50 p-4">
              <Card className="w-full max-w-sm md:max-w-[400px]">
                <CardHeader>
                  <CardTitle className="text-lg md:text-xl">{t("zoneImport.complete")}</CardTitle>
            ...
            <div className="space-y-4 md:space-y-6">
              <div className="flex items-center justify-center p-4 md:p-8">
                <div className="text-center space-y-3 md:space-y-4">
                  ...
                  <h3 className="text-xl md:text-2xl font-medium">{t("zoneImport.complete")}</h3>
                  <p className="text-xs md:text-sm text-muted-foreground">
                    {parsedData.length} {t("zoneImport.succesImport")}
                  </p>
                </div>
          ...
          {step === 5 && (
            <div className="space-y-4 md:space-y-6">
              <div className="space-y-1 md:space-y-2">
                <h3 className="text-base md:text-lg font-medium">{t("zoneImport.zoneImporter")}</h3>
                <p className="text-xs md:text-sm text-muted-foreground">{t("zoneImport.listeZones")}</p>
              </div>
              ...
              {showAddForm && (
                <Card className="w-full">
                  <CardHeader>
                    <CardTitle className="text-lg md:text-xl">{t("zoneImport.formulaire.addZone")}</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3 md:space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                      <div className="space-y-1 md:space-y-2">
                        <label className="text-xs md:text-sm font-medium">{t("zoneImport.headers.zone_id")}*</label>
                        <Input
                          name="zone_id"
                      ...
                      <div className="space-y-1 md:space-y-2">
                        <label className="text-xs md:text-sm font-medium">{t("zoneImport.headers.nom_zone")}*</label>
                        <Input
                          name="nom_zone"
                      ...
                      <div className="space-y-1 md:space-y-2 md:col-span-2">
                        <label className="text-xs md:text-sm font-medium">
                          {t("categoryImport.headers.magasin_id")}*
                        </label>
                      ...
                      <div className="space-y-1 md:space-y-2">
                        <label className="text-xs md:text-sm font-medium">
                          {t("zoneImport.formulaire.zoneNameDescription")}
                        </label>
                        <Input
                          name="description"
                      ...
                      <div className="space-y-1 md:space-y-2">
                        <label className="text-xs md:text-sm font-medium">{t("locationTitle")}</label>
                        <Input
                          name="emplacement"
                    ...
                    <Button onClick={handleAddZone} className="mt-3 md:mt-4 text-sm md:text-base">
              ...
              <div className="space-y-2">
                <h4 className="font-medium text-sm md:text-base">{t("zoneImport.zoneImporter")}</h4>
              ...
              <div className="flex justify-between mt-4">
                ...
                <Button onClick={() => (window.location.href = "/zones")} className="text-sm md:text-base">
                  {t("zoneImport.viewZone")}
                </Button>
              </div>
            </div>
