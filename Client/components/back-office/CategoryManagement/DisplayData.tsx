...
const DisplayData: React.FC = () => {
  ...
  const fetchData = async (entrepriseId?: number) => {
    try {
      ...
      let currentEntrepriseId = entrepriseId
      ...
      if (!currentEntrepriseId) {
        ...
        currentEntrepriseId = userData.entreprises_id
      }
      ...
      console.log(`Récupération des magasins pour l'entreprise ${currentEntrepriseId}...`) // Debug
      const magasinsRes = await axios.get(`${API_BASE_URL}/magasins/getMagasinsByEntrepriseId/${currentEntrepriseId}`)
  ...
  const openEditModal = (type: "magasin" | "categorie" | "zone", record: any) => {
  ...
  const saveNewItem = async () => {
    ...
    try {
      ...
      if (editingType === "magasin") {
        values.entreprise_id = currentUser.entreprises_id // Correction ici
      }
      ...
      if (response.data && response.status === 201) {
        ...
        await fetchData(currentUser.entreprises_id) // Passer l'ID de l'entreprise
  ...
  const saveModalEditing = async () => {
    try {
      ...
      fetchData(currentUser?.entreprises_id) // Recharger les données après modification
    }
  }
    ...
    if (editMode && editingId === record.id && editingField === field && editingType === type) {
      return (
        <div className="editable-cell-input-container">
          ...
          />
          <Button icon={<CloseOutlined />} size="small" onClick={cancelEditing} className="editable-cancel-btn" />
        </div>
      )
    }
    return (
      <div onDoubleClick={() => editMode && handleCellClick(type, record, field)} className="editable-cell">
  ...
  const handleCellClick = (type: "magasin" | "categorie" | "zone", record: any, field: string) => {
    startEditing(type, record.id, field, record[field])
  }

  const startEditing = (type: "magasin" | "categorie" | "zone", id: number, field: string, value: any) => {
    setEditingType(type)
    setEditingId(id)
  ...
  const handleDelete = async (type: "magasin" | "categorie" | "zone", id: number) => {
    ...
    const isConfirmed = window.confirm(t("category3Managment.displayData.confirmDelete"))
    ...
    try {
      ...
      notification.success({
        message: t("category3Managment.displayData.success"),
        description: t("category3Managment.displayData.deleteSuccess"),
      })
    } catch (error) {
      console.error("Erreur:", error)
      notification.error({
        message: t("category3Managment.displayData.error"),
        description: t("category3Managment.displayData.deleteFailed"),
      })
    }
  }
  ...
  const toggleEditMode = () => {
    ...
    notification.info({
      message: `Mode édition ${editMode ? t("category3Managment.displayData.disabled") : t("category3Managment.displayData.enabled")}`,
      description: editMode
        ? t("category3Managment.displayData.editModeOffMessage")
        : t("category3Managment.displayData.editModeOnMessage"),
    })
  }
  ...
  const magasinsColumns = [
    ...
    },
    {
      ...
      key: "actions",
      fixed: "right" as const, // Explicitly cast to 'right'
      width: 150,
      render: (_: any, record: Magasin) => (
        <Space size="small">
          <Button
            ...
            aria-label={t("category3Managment.displayData.edit")}
          />
          <Button
            danger
            onClick={() => handleDelete("magasin", record.id)}
            icon={<DeleteOutlined />}
            aria-label={t("category3Managment.displayData.supprimer")}
          />
        </Space>
      ),
    },
    ...
    switch (editingType) {
      case "magasin":
        return (
          <>
            <Form.Item
              label={t("category3Managment.displayData.nomDelMagasin")}
              name="nom_magasin"
              rules={[{ required: true }]}
            >
              <Input />
            ...
            <Form.Item
              label={t("category3Managment.displayData.surface")}
              name="surface"
      ...
      case "categorie":
        return (
          <>
            <Form.Item
              label={t("category3Managment.displayData.nomDelCategorie")}
              name="nom"
              rules={[{ required: true }]}
            >
              <Input />
      ...
      case "zone":
        return (
          <>
            <Form.Item
              label={t("category3Managment.displayData.nomDelZone")}
              name="nom_zone"
              rules={[{ required: true }]}
            >
              <Input />
            ...
            <Form.Item
              label={t("category3Managment.displayData.magasin")}
              name="magasin_id"
              rules={[{ required: true }]}
            >
              <Select>
    ...
    switch (editingType) {
      case "magasin":
        return (
          <>
            <Form.Item
              label={t("category3Managment.displayData.idMagasin")}
              name="magasin_id"
              rules={[{ required: true }]}
            >
              <Input />
            </Form.Item>
            <Form.Item name="entreprise_id" initialValue={currentUser?.entreprises_id} hidden>
              <Input />
            </Form.Item>
            <Form.Item
              label={t("category3Managment.displayData.nomDelMagasin")}
      ...
      case "categorie":
        return (
          <>
            <Form.Item
              label={t("category3Managment.displayData.idCategorie")}
              name="categorie_id"
              rules={[{ required: true }]}
            >
              <Input />
            ...
            <Form.Item label={t("categoryImport.headers.parent_id")} name="magasin_id">
              {" "}
              {/* Changed to magasin_id */}
              <Select>
                <Select.Option value="">{t("category3Managment.displayData.aucune")}</Select.Option>
                {magasins.map((mag) => (
                  <Select.Option key={mag.magasin_id} value={mag.magasin_id}>
  ...
  return (
    <div className="display-data-container" dir={textDirection}>
      <Link href="/Dashboard" passHref>
        <Button type="primary" className="back-to-dashboard-btn" icon={<ArrowLeftOutlined />}>
        ...
        <div className="section-header">
          ...
          <div className="section-actions">
            ...
            <Switch
              ...
              className="toggle-visibility-switch"
            />
          </div>
        </div>
        {showMagasins && (
