...
const handleCancelEdit = () => {
  ...
  const importCategories = () => {
    const validCategories = parsedData.map((row) => {
      ...
      Object.entries(columnMapping).forEach(([originalCol, mappedCol]) => {
        if (mappedCol && row[originalCol] !== undefined && row[originalCol] !== null) {
          ...
          if (mappedCol === "categorie_id") {
            ...
            mappedCategory.parent_id = row[originalCol] === "-" || row[originalCol] === "" ? undefined : String(row[originalCol]);
          } else {
          ...
          {step === 1 && (
            <div className="space-y-6">
              <div className="space-y-2">
                <h3 className="text-lg font-medium"> {t("categoryImport.title")}</h3>
                <p className="text-sm text-muted-foreground">
                  {t("categoryImport.description")}
                </p>
              </div>
              ...
              >
                {file ? (
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-muted-foreground">{parsedData.length} catégories détectées</p>
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={(e) => {
      ...
      <div className="p-4 min-w-max">
        ...
        {parsedData.length > 5 && (
            ...
            + {parsedData.length - 5} {t("categoryImport.columnsStep.otherCategories")}
              ...
              {validationErrors.length > 0 && (
                <Alert variant="destructive">
                  ...
                  <AlertTitle>{t("categoryImport.validation.errors")}</AlertTitle>
                  <AlertDescription>
          ...
          {step === 3 && (
            <div className="space-y-6">
              <div className="space-y-2">
                <h3 className="text-lg font-medium">{t("categoryImport.importStep.title")}</h3>
                <p className="text-sm text-muted-foreground">
                  {t("categoryImport.importStep.description")}
                </p>
              </div>

              <div className="space-y-4">
                <h4 className="font-medium">{t("categoryImport.importStep.summary")}</h4>
                <div className="border rounded-md p-4">
                  <p className="text-sm text-muted-foreground">{t("categoryImport.importStep.categoriesToImport")}</p>
              ...
              <div className="flex justify-between">
                ...
                <Button onClick={importCategories}>{t("categoryImport.importCategories")}</Button>
              </div>
            </div>
          )}
...
{step === 5 && (
  <div className="space-y-6">
    <div className="space-y-2">
      <h3 className="text-lg font-medium">{t("categoryImport.validCategoryImport")}</h3>
      <p className="text-sm text-muted-foreground">
