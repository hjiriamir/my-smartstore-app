...
export function ProductLibrary() {
  ...
  const uniqueCategoryIds = [
    ...new Set(
      products.flatMap((product) => {
        ...
        if (product.category1_id) ids.push(product.category1_id)
        if (product.category2_id) ids.push(product.category2_id)
        if (product.category3_id) ids.push(product.category3_id)
        return ids
      }),
    ),
  ]
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchesSearch =
      ...
      (product.primary_id ? product.primary_id.toLowerCase().includes(searchTerm.toLowerCase()) : false) ||
      (product.primary_Id ? product.primary_Id.toLowerCase().includes(searchTerm.toLowerCase()) : false) ||
    ...
    // Supplier filter
    const matchesSupplier = !selectedSupplier || selectedSupplier === "all" || product.supplier === selectedSupplier
    ...
    <div className={`container max-w-6xl mx-auto py-6 ${isRTL ? "rtl" : "ltr"}`}>
      ...
      <div className="flex flex-col space-y-6">
            ...
            <AlertDialog>
              ...
              <AlertDialogContent className={isRTL ? "text-right" : "text-left"}>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>{t("productImport.clearLibraryWarning")}</AlertDialogDescription>
                </AlertDialogHeader>
        ...
        <div className="flex flex-col space-y-4">
          {/* Filters */}
          <div
            className={`flex flex-col space-y-4 md:flex-row md:space-y-0 md:space-x-4 ${isRTL ? "md:space-x-reverse" : ""}`}
          >
          ...
          >
            {paginatedProducts.length > 0 ? (
              paginatedProducts.map((product) => {
                ...
                const mainCategory = getCategoryById(product.category_id)
              ...
              <div
                className={`col-span-full text-center py-12 text-muted-foreground ${isRTL ? "text-right" : "text-left"}`}
              >
